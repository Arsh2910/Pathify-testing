...
const MilestoneCard = ({ milestone, phaseId }) => {
  const toggleMilestoneCompletion = useStore(
    (state) => state.toggleMilestoneCompletion,
  );
  const setRoadmapProgress = useStore((state) => state.setRoadmapProgress);
  ...
  const handleToggle = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await apiClient.patch(`/milestones/${milestone._id}`, {
        isCompleted: newStatus,
      });
      const { progress, streak } = response.data.data;
        ...
        <div className="milestone-main-col">
          ...
          <div className="anti-procrastination-block">
            <div className="why-now">
              <strong>
                <Zap size={16} className="inline-icon text-accent" /> Why Now:
              </strong>{" "}
              {milestone.whyNow}
            </div>
            ...
            <div className="micro-step">
              ...
              <span className="micro-step-text">
