...
const milestoneSchema = {
  ...
  items: {
    ...
    properties: {
      ...
      microFirstStep: { type: Type.STRING, description: "A tiny, non-intimidating first action to start the milestone and beat procrastination" },
      whyNow: { type: Type.STRING, description: "Short motivational context for why this step matters at this point in the roadmap" },
      suggestedTimeBox: { type: Type.STRING, description: "Estimated minutes/hours (e.g., '30 minutes', '2 hours')" },
      resources: {
        type: Type.ARRAY,
        items: {
          type: Type.OBJECT,
          properties: {
            ...
            type: { type: Type.STRING, enum: ['video', 'article', 'course', 'book', 'other'] }
          },
          required: ['title', 'link', 'type']
        }
      }
    },
    required: ['title', 'description', 'microFirstStep', 'whyNow', 'suggestedTimeBox', 'resources']
  }
};
 ...
 */
exports.generateMilestonesForPhase = async (phaseTitle, goal, userPreferences, timeframe) => {
  const { skillLevel, hoursPerDay } = userPreferences;

  const prompt = `You are an expert learning roadmap generator.
